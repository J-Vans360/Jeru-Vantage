...
import assessmentData from '@/data/part-a-s2-values.json';
import { calculateValuesScores } from '@/lib/scoring';
import ResultsDisplay from '@/components/assessment/ResultsDisplay';

const USER_ID = 'test-user-123';

export default function PartAS2Page() {
  ...
  const handleComplete = async (finalAnswers: Record<number, number>) => {
    const calculatedScores = calculateValuesScores(finalAnswers, assessmentData);
  ...
  if (isComplete && scores) {
    return (
      <ResultsDisplay
        ...
        scores={scores}
        domains={domains}
        onContinue={() => router.push('/assessment/part-a-s3')}
        continueButtonText="Continue to Part A: S3 (Holland Code) →"
        isSaving={isSaving}
      />
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div
              className="bg-gradient-to-r from-pink-600 to-red-600 h-3 rounded-full transition-all duration-300"
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="px-6 py-3 rounded-lg bg-pink-600 text-white font-medium hover:bg-pink-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            Next →
          </button>
        </div>
