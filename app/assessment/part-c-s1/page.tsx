...
export default function PartCS1Page() {
  ...
  const handleComplete = async (finalAnswers: Record<number, number>) => {
    const calculatedScores = calculateEnvironmentScores(finalAnswers, assessmentData);
  ...
  if (isComplete && scores) {
    return (
      <EnvironmentResultsDisplay
        section={section}
        scores={scores}
        domains={domains}
        onContinue={() => router.push('/assessment/part-c-s2')}
        continueButtonText="Continue to Part C: S2 (Execution & Grit) →"
        isSaving={isSaving}
      />
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="px-6 py-3 rounded-lg bg-emerald-600 text-white font-medium hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            Next →
          </button>
        </div>
