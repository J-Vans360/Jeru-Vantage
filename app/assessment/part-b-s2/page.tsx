...
import assessmentData from '@/data/part-b-s2-stress.json';
import { calculateStressScores } from '@/lib/scoring';
import StressResultsDisplay from '@/components/assessment/StressResultsDisplay';


export default function PartBS2Page() {
  ...
  const handleComplete = async (finalAnswers: Record<number, number>) => {
    const calculatedScores = calculateStressScores(finalAnswers, assessmentData);
  ...
  if (isComplete && scores) {
    return (
      <StressResultsDisplay
        ...
        scores={scores}
        domains={domains}
        stressInfo={assessmentData.stressInfo}
        onContinue={() => router.push('/assessment/part-b-s3')}
        continueButtonText="Continue to Part B: S3 (21st Century Skills) →"
        isSaving={isSaving}
      />
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="px-6 py-3 rounded-lg bg-teal-600 text-white font-medium hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            Next →
          </button>
        </div>
