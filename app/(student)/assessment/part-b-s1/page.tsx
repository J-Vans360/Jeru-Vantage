...
export default function PartBS1Page() {
  ...
  const handleComplete = async (finalAnswers: Record<number, number>) => {
    const calculatedScores = calculateCognitiveScores(finalAnswers, assessmentData);
  ...
  if (isComplete && scores) {
    return (
      <CognitiveResultsDisplay
        section={section}
        scores={scores}
        domains={domains}
        onContinue={() => router.push('/assessment/part-b-s2')}
        continueButtonText="Continue to Part B: S2 (Stress Response) →"
        isSaving={isSaving}
      />
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="px-6 py-3 rounded-lg bg-purple-600 text-white font-medium hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            Next →
          </button>
        </div>
